...
### Resolved implementation details (authoritative)

These decisions refine ambiguous wording elsewhere in this document.

1. **Retired-fact drop vs `explains_active_truth` (not time-based)**  
   After retirement, **drop** (do not keep in the retired bucket) only if the fact is **low-value** (`context` + subtype `other`, or `preference` + subtype `other`) **and** **no active fact in that section** lists this retired fact’s `memory_id` in its `supersedes` field.  
   If some active fact’s `supersedes` includes this `memory_id`, the retired fact is **not** eligible for drop (same predicate as **`explains_active_truth == 1`** in the retired-cap eviction sort). There is **no** time window; “recently” in older drafts meant **identity**, not recency.

2. **Single-batch candidate ordering (two-pass partition, stable order)**  
   Process the candidate list in **two passes**; preserve **original list order** within each pass. **No** fixed-point iteration.
   - **Pass 1:** All `retire` ops; all `upsert` ops whose `supersedes_fact_keys` **resolve** to at least one **active** fact in the section; all **`confirm`** ops; all **update** `upsert`s (with `target_id`).  
   - **Pass 2:** All remaining **new-create** `upsert`s (no `target_id`) that either omit `supersedes_fact_keys` or whose keys **do not** resolve to active facts.  
   Two passes are sufficient: retirements and superseding upserts free slots before non-superseding new creates; no retirement depends on another retirement’s outcome in a way that requires a loop.

3. **`get_memory_notes` → `get_sectioned_memory()` rename scope**  
   **Email service only** (`sam-app/email_service/`). Action-link handler and registration lambda do not read/write athlete memory. **Hard cut:** rename at call sites, **no** deprecated alias. Grep at cutover to confirm.

4. **`priority_facts` size (compiler)**  
   The compiler includes **all** active goals and **all** active constraints—**no** cap inside `memory_compiler.py`. The **implicit** upper bound is from **storage** active caps: at most **4** goals + **8** constraints = **12** `priority_facts`.

5. **`preference_facts` in `memory_payload` (PR 5)**  
   Same pattern as other lists: **omit** the key when the list is empty. The response-generation prompt should treat **`preference_facts` as optional**—if present, use it; if absent, no preference lines apply (same spirit as `context_facts` today). Add prompt text alongside existing keys in PR 5.

6. **Reject-at-cap logging**  
   Use `logging.getLogger(__name__)` and **`logger.warning(...)`** with structured context: section name, rejected candidate summary, current active count, section cap. **No** extra metrics or alerting—warnings only (e.g. CloudWatch). Match the pattern the **current** reducer uses for overflow-style warnings.

7. **Tests: unit vs e2e**  
   Run the **`sam-app/email_service` unit suite** on every PR. **`sam-app/e2e/test_live_endpoints.py`** (live AWS) is only meaningful **after PR 5** wires the pipeline; **do not block PRs 1–4** on e2e. After PR 5 (and for the merge bar), run the full suites per `AGENTS.md`.
...
The `memory_notes` field on `coach_profiles` currently holds a flat list of fact dicts. After this change it should hold the new sectioned structure. The `replace_memory()` and `get_memory_notes()` functions in `dynamodb_models.py` need to serialize/deserialize the new shape. At cutover, rename **`get_memory_notes()`** to **`get_sectioned_memory()`** (and update **email_service** call sites only—see **Resolved implementation details** above).
...
- `sam-app/email_service` unit suite passes: `python3 -m unittest discover -v -p "test_*.py" -s sam-app/email_service` (**do not block this PR on live e2e**—see **Resolved implementation details**)
...
4. **Implement retired fact routing.** After each retirement, decide destination:
   - **Drop** if: the fact is low-value (`context` + subtype `other`, or `preference` + subtype `other`) **and** **no** active fact in the section lists this retired fact’s `memory_id` in `supersedes` (same as `explains_active_truth == 0`; **not** time-based)
...
6. **Implement active section cap enforcement via reject-at-cap.** When processing a new-create upsert, if the target section's active list is already at cap:
   ...
   - **If the upsert does not supersede any existing active fact**: reject the new upsert. **`logger.warning`** with structured context: section name, rejected candidate summary, current active count, section cap (see **Resolved implementation details**). Do not store the fact.
   ...
   - **Processing order:** apply the **two-pass partition** in **Resolved implementation details** (Pass 1: retires, superseding upserts, confirms, update-upserts; Pass 2: remaining new-create upserts). Slots freed in Pass 1 are available for Pass 2 in the same batch.
...
- Retired fact routing: low-value facts are dropped only when no active `supersedes` points at them; meaningful / referenced facts are kept
...
- Two-pass ordering: Pass 1 (retire + superseding upserts + confirm + update-upsert) then Pass 2 (remaining new-create upserts), stable order within each pass
...
- `sam-app/email_service` unit suite passes (**do not block on live e2e**)
...
3. **Compiler budget (constants):**
   ...
   **Implicit bound on `priority_facts`:** There is **no** compiler-side cap on goals or constraints, but storage active caps mean at most **4** goals + **8** constraints = **12** items in `priority_facts`.

4. **Safety rule:** If total active goals + constraints is large relative to the bounded lower sections, the compiler still includes all goals and constraints. It trims `context_notes` first, then preferences, then schedule_anchors. Goals and constraints are never trimmed.
...
- **No existing files modified.** `sam-app/email_service` unit suite passes (**do not block on live e2e**)
...
#### `dynamodb_models.py` (email service only—no memory access in other Lambdas)

1. **Rename `get_memory_notes()` to `get_sectioned_memory()`** and update all **email_service** call sites (hard cut, no alias):
...
2. **Update `build_response_brief()`:**
   ...
   - The new `preference_facts` key is added because preferences are now a separate compiler section rather than grouped into `context_facts`. **Omit `preference_facts` when empty.** Update the response-generation prompt to mention `preference_facts` alongside existing keys; treat it as **optional**—if the key is absent, the model has no preference lines to apply (same optional pattern as `context_facts`).
...
4. **The response generation skill prompt** (`skills/response_generation/`): verify and update so `preference_facts` is documented as optional (omit when empty), consistent with other memory lists.
...
After this cutover, run the **full merge bar** (`AGENTS.md`: `action_link_handler` + `email_service` unit suites + **`sam-app/e2e/test_live_endpoints.py`**). Specifically:
...
- **`sam-app/email_service` unit suite passes.** Run the **full merge bar** from `AGENTS.md` (including `action_link_handler` unit tests and **`sam-app/e2e/test_live_endpoints.py`**) when validating this cutover PR—e2e is meaningful now that the pipeline is wired.
...
3. **Verify `sam-app/email_service` unit suite passes** (and full merge bar if not already green after PR 5).

### Definition of done

- CLAUDE.md files reflect new system
- Regression tests for bugs 19, 6, 2 pass
- `sam-app/email_service` unit suite passes; follow `AGENTS.md` for full merge bar when preparing release
...
**Critical invariant: every PR leaves the `sam-app/email_service` unit suite green.** PRs 1–4 are purely additive — they create new files and tests without touching existing code — and **must not be blocked on `sam-app/e2e/test_live_endpoints.py`** (live AWS). PR 5 is the atomic cutover; run the **full merge bar** in `AGENTS.md` (including e2e) when validating PR 5 / release readiness.
