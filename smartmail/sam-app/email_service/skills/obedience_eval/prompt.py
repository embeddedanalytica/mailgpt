...
4. **exceeded_requested_scope** — The directive is narrow (few content_plan items, short main_message) but the email is disproportionately long, adds bonus tips, unsolicited summaries, or stale follow-ups. Specific sub-patterns to catch:
...
6. **missed_exact_instruction** — The directive specifies a required answer, explicit athlete instruction, or safety-critical detail that the email fails to address.
...
- Produce a corrected_email_body that fixes ALL violations while:
  ...
  - For exceeded_requested_scope: trim the email to the minimum sufficient answer. Remove restated standing rules, recap sections, stale follow-ups, and boilerplate reminders.
  - For introduced_unsupported_assumption: remove unsupported week/block labels.
  - For ignored_latest_constraint: adjust the email only as much as needed to comply with the constraint.
