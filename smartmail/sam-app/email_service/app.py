...
def lambda_handler(event, context):
    """Lambda function to receive an email via SNS, generate a response via OpenAI, and reply using SES."""
    try:
        ...
        reply_content = generate_message(email_data["subject"], email_data["body"])

        # Send the reply email
        message_id = send_reply(event, reply_content, email_data["body"])
...
def send_reply(event, reply_content, original_body):
    ...
    sender_email = ses_data["mail"]["source"]  # Original sender (who emailed you)
    recipient_email = ses_data["mail"]["destination"][0]  # Who the sender emailed
    original_subject = ses_data["mail"]["commonHeaders"]["subject"]
    original_message_id = ses_data["mail"]["commonHeaders"]["messageId"]

    # Extract base64-encoded original content
    original_content_base64 = ses_data.get("content", "")
    if original_content_base64:
        original_content = base64.b64decode(original_content_base64).decode("utf-8", errors="ignore")
    else:
        original_content = "Original message not available."

    # Format reply body (include original email for context)
    formatted_reply = f"""

{reply_content}

---
On {ses_data["mail"]["commonHeaders"]["date"]}, {sender_email} wrote:

{original_body}
    """

    # Create email message with threading support
    msg = MIMEMultipart()
    msg["Subject"] = f"Re: {original_subject}"  # Ensure "Re:" is added for threading
    ...
    msg["In-Reply-To"] = original_message_id  # Links reply to original email
    msg["References"] = original_message_id  # Important for threading
