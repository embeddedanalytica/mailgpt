...
TURNS: List[TurnSpec] = [
    TurnSpec(1, "Starting over after a layoff", "Hi coach, I want to get back to running after a messy few months. My big goal is to get healthy enough to race a half marathon again this spring. Right now I mostly need help rebuilding without doing something stupid.", "freeform", "clarification_or_inference_ok", expected_intent="question", expect_clarification_semantics=True),
    TurnSpec(2, "A little more context", "A bit more context: I used to run consistently before the layoff and I'd call myself intermediate. I can train four days a week most weeks if we keep it realistic. The thing that nags me is mild Achilles tightness when I get greedy. I also do better when the plan feels structured.", "freeform", "clarification_or_inference_ok", acceptable_intents={"question", "availability_update"}, expect_memory=True),
    ...
    TurnSpec(10, "Tune-up effort felt controlled", "Small milestone: I did a tune-up 5k effort in 24:50 and it felt controlled. I added a 20m cool-down jog after. No big pain spike, just normal post-workout heaviness, and sleep was good.", "semi_structured", "clarification_or_inference_ok", expected_intent="milestone_update", expect_manual_snapshot=True, expect_memory=True, expect_progress_non_default=True, allow_clarification_instead_of_inference=True, inference_candidate=True),
    TurnSpec(11, "Still on the same four-day pattern", "Just to confirm, I'm still doing four days per week most weeks. The same general Monday Wednesday Friday Sunday pattern still works, although Friday has become the easiest day for me to protect.", "freeform", "clarification_or_inference_ok", acceptable_intents={"availability_update", "question"}, expect_memory=True, expect_durable_schedule_reuse=True),
    TurnSpec(12, "Travel week coming up", "Travel week heads-up: next week is rough. I only have Tuesday, Thursday, and Saturday open, and hotel treadmill plus a tiny gym are all I'll have. I'd love the simplest travel-friendly version of the plan.", "freeform", "clarification_or_inference_ok", acceptable_intents={"plan_change_request", "availability_update"}, expect_memory=True, expect_clarification_semantics=True),
    TurnSpec(13, "Travel details", "Event date: 2026-05-17. Days available: 3. Pain score: 2 out of 10. Risk candidate is yellow because I am traveling for work. Week is chaotic true. Equipment access is treadmill and gym, no track. I can do 40m treadmill runs and one short strength session.", "structured", "should_mutate", acceptable_intents={"plan_change_request", "availability_update"}, expect_plan_growth=True),
    TurnSpec(14, "Hotel treadmill check-in", "Travel check-in: I did a run for 40m on the hotel treadmill, about 4.5 miles, avg hr 146, and roughly 520 calories. Felt decent, slept ok, and nothing got worse. Mostly just mentally flat from travel.", "semi_structured", "clarification_or_inference_ok", expected_intent="check_in", expect_manual_snapshot=True, expect_progress_non_default=True, allow_clarification_instead_of_inference=True, inference_candidate=True),
    TurnSpec(15, "Back home and Saturday is open", "Good news: for the next month Saturday is open again, so I have more flexibility for the long run. Week to week I still want to stay at four days, but now the long run doesn't have to be Sunday every time.", "freeform", "clarification_or_inference_ok", acceptable_intents={"availability_update", "plan_change_request"}, expect_memory=True, expect_durable_schedule_reuse=True),
    TurnSpec(16, "Family chaos week", "This week got blown up by family stuff. I only got in one 30m run and I'm feeling low energy today. Can we just simplify everything for a few days and keep the important things important?", "freeform", "clarification_or_inference_ok", acceptable_intents={"check_in", "plan_change_request", "availability_update"}, expect_memory=True, expect_clarification_semantics=True),
    TurnSpec(17, "Details for the family-chaos week", "Event date: 2026-05-17. Days available: 2. Pain score: 3 out of 10. Risk candidate is yellow. Family stuff blew up this week. I only got in a 30m run. Felt low energy, sore, poor sleep. Stress score is 9. I am worried about losing momentum.", "structured", "should_mutate", expected_intent="check_in", expect_manual_snapshot=True, expect_memory=True, expect_progress_non_default=True),
    TurnSpec(18, "Back on track", "Better week. I ran three times: 35m easy, 50m steady, and 80m today for about 9 miles. Felt good, slept well, and the Achilles feels quiet again. Same four-day rhythm still seems like the sweet spot.", "semi_structured", "clarification_or_inference_ok", expected_intent="check_in", expect_manual_snapshot=True, expect_memory=True, expect_progress_non_default=True, allow_clarification_instead_of_inference=True, inference_candidate=True, expect_durable_schedule_reuse=True),
    ...
    TurnSpec(25, "Mild cold and weather", "I have a mild cold and the weather is awful this week. Energy is not great. I don't want to do anything dumb, but I also don't want to lose the thread completely. How conservative should I be?", "freeform", "clarification_or_inference_ok", acceptable_intents={"plan_change_request", "question", "check_in"}, expect_clarification_semantics=True),
    TurnSpec(26, "Illness details", "Event date: 2026-05-17. Days available: 3. Pain score: 2 out of 10. Risk candidate is yellow. Recent illness is mild. Energy score is 4. Sleep score is 5. I want to know how conservative I should be this week.", "structured", "should_mutate", expected_intent="plan_change_request", expect_plan_growth=True, expect_memory=True),
    TurnSpec(27, "Half marathon done", "Race update: I ran the half marathon today in 1:55. I jogged 15m after, I feel tired but happy, and nothing feels scary. This whole build really confirmed that four days per week is a good groove for me.", "semi_structured", "clarification_or_inference_ok", expected_intent="milestone_update", expect_manual_snapshot=True, expect_memory=True, expect_progress_non_default=True, expect_durable_schedule_reuse=True),
    ...
    TurnSpec(29, "One more durable schedule note", "One thing I learned from this block: four days is sustainable for me, and Saturday is probably the best long-run anchor going forward. Fridays still need to stay pretty light because of work.", "freeform", "clarification_or_inference_ok", acceptable_intents={"availability_update", "question", "milestone_update"}, expect_memory=True, expect_durable_schedule_reuse=True),
    TurnSpec(30, "Freeform final check-in", "Quick final check-in before I go quiet for a few days: easy 35m jog today, about 4 miles, avg hr 138, felt good, slept well, and I'm mostly just happily tired. Curious if your advice changes much given everything you've seen from me over this stretch.", "freeform", "clarification_or_inference_ok", acceptable_intents={"check_in", "question", "milestone_update"}, expect_manual_snapshot=True, expect_memory=True, expect_progress_non_default=True, allow_clarification_instead_of_inference=True, inference_candidate=True, expect_durable_schedule_reuse=True),
]
...
class TestLiveCoachingWorkflow(unittest.TestCase):
    ...
    def test_live_thirty_turn_coaching_workflow(self) -> None:
        ...
        with mock.patch.object(email_reply_sender.ses_client, "send_raw_email", side_effect=self.capture.send_raw_email):
            for turn in TURNS:
                ...
                self.turn_results[-1].update({
                    ...
                    "memory_notes_count": len(memory_context.get("memory_notes", []) or []),
                ...
                if turn.expect_memory:
                    has_memory = bool(memory_context.get("memory_notes") or memory_context.get("continuity_summary"))
        ...
        self.assertTrue(final_memory_context.get("memory_notes") or final_memory_context.get("continuity_summary"), "final memory context remained empty")
