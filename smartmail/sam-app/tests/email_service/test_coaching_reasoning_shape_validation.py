...
class TestCoachingReasoningShapeValidation(unittest.TestCase):
    def test_answer_first_turn_accepts_concise_directive(self):
        result = validate_coaching_directive(
            _valid_directive(),
            response_shape="answer_first_then_stop",
            turn_purpose="lightweight_answer",
        )
        self.assertEqual(result["content_plan"], ["Answer the question directly", "Give one short reason"])
