...
import sectioned_memory_contract  # type: ignore
...
def _sectioned_active_fact_count(memory_context: Dict[str, Any]) -> int:
    """Count active facts across all sectioned buckets (Dynamo `memory_notes` JSON)."""
    sm = memory_context.get("sectioned_memory")
    if not isinstance(sm, dict):
        return 0
    n = 0
    for bucket in sectioned_memory_contract.VALID_STORAGE_BUCKETS:
        active = (sm.get(bucket) or {}).get("active") or []
        if isinstance(active, list):
            n += len(active)
    return n


def _has_memory_artifacts(memory_context: Dict[str, Any]) -> bool:
    if memory_context.get("continuity_summary"):
        return True
    return _sectioned_active_fact_count(memory_context) > 0
...
TURNS: List[TurnSpec] = [
    ...
    TurnSpec(
        26,
        "Illness details",
        "Event date: 2026-05-17. Pain score: 2 out of 10. Risk candidate is yellow. Recent illness is mild. Energy score is 4. Sleep score is 5. I want to know how conservative I should be this week.",
        "structured",
        "should_mutate",
        acceptable_intents={"coaching", "question", "safety_concern"},
        acceptable_requested_actions={"plan_update", "answer_question"},
        expect_plan_growth=True,
        expect_memory=True,
    ),
...
class TestLiveCoachingWorkflow(unittest.TestCase):
    ...
    def test_live_thirty_turn_coaching_workflow(self) -> None:
        ...
        with mock.patch.object(email_reply_sender.ses_client, "send_raw_email", side_effect=self.capture.send_raw_email):
            for turn in TURNS:
                ...
                self.turn_results[-1].update({
                    ...
                    "sectioned_active_fact_count": _sectioned_active_fact_count(memory_context),
                ...
                if turn.expect_memory:
                    self.assertTrue(
                        _has_memory_artifacts(memory_context),
                        f"turn {turn.number} expected memory artifacts to exist",
                    )
        ...
        self.assertTrue(
            _has_memory_artifacts(final_memory_context),
